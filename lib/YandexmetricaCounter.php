...
final class YandexmetricaCounter {
    ...
    function __construct($path) {
        ...
        $this->plugin_title = __('Wordefinery Yandex.Metrica Counter');
        ...
        if ($this->store->site_id) {
            ...
            switch ($this->informer->mode) {
                case 'widget':
                    \add_action('widgets_init', function () { \register_widget('\\wordefinery\\YandexmetricaCounterWidget'); });
                    break;
                case 'footer':
                    break;
                case 'shortcode':
...
//        \add_action('wp_ajax_check_site_id', array(&$this, 'SettingsCheckSiteId'));
    ...
    function AdminMenu() {
        ...
        \add_action( 'admin_print_styles-' . $page, function () use ($slug) { \wp_enqueue_style('wordefinery-tabs'); \wp_enqueue_style( 'farbtastic' ); \wp_enqueue_style($slug.'-ui-slider'); \wp_enqueue_style($slug.'-settings'); } );
    ...
    function SettingsPage() {
        ...
        $tabs = array(''=>__('Counter'), 'informer'=>__('Informer'), /* 'shortcodes'=>__('Shortcodes') */ );
        include(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $this->path . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'settings-page-tabs.php');
        switch ($tab) {
            case 'shortcodes':
                include(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $this->path . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'yandexmetricacounter-settings-page-shortcodes-tab.php');
                break;
            case 'informer':
                include(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $this->path . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'yandexmetricacounter-settings-page-informer-tab.php');
                break;
            default:
                include(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $this->path . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'yandexmetricacounter-settings-page.php');
                break;
        }
    }
